...
export function createProbeRunner({ db, emit, now = () => new Date(), clock = () => performance.now() }) {
  ...
  return async function runProbe() {
    ...
    try {
      ...
      const resultSummary = {
        mysql2Tuple: Array.isArray(clientProbe.result) && clientProbe.result.length === 2,
        rowsPresent: Array.isArray(clientProbe.result[0]),
        fieldsPresent: Array.isArray(clientProbe.result[1]),
        rowCount: rows.length,
        fieldCount: clientProbe.result[1].length,
      };
